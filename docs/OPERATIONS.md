...
The existing Host storage backend owns canonical domain files. The domain name includes a hash of the memory home, isolating distinct homes even when they share a backend. A root-derived loopback OS lease prevents two live profiles in one home from writing concurrently. It creates no persistent lock file and is released by the kernel on clean shutdown, forced termination or process crash. A bounded set of deterministic loopback ports handles identified unrelated lease collisions; an ambiguous occupied port or exhaustion fails closed. Use separate homes and backend roots for separate owners. The optional MCP mode owns a separate bounded JSON backend and refuses the Host directory. Other privileged plugins and the OS account are inside the trusted Host boundary.
...
Jobs are bounded, persistent and idempotent by session/event range. A worker processes one job at a time, retries up to three attempts with backoff, and expires work after seven days. Missing model route, disabled egress and insufficient daily budget remain pending with reasons. A job's previous reservation is retained after interrupted execution. Pause, dependency removal and plugin disposal cancel work; late provider results cannot publish after their owner stops. Removal drains operations before releasing the ownership lease. A provider that ignores cancellation may still incur remote cost after its request is abandoned; its late result has no commit authority.

No stale-lock recovery is required after a crash: the operating system releases the ownership lease with the dead process. If startup reports `store-owned`, another live process owns the same store or all deterministic loopback lease ports are occupied. Stop the other owner or use a separate home and backend root; do not bypass live ownership. Pending/running jobs resume with preserved reservations on restart.
