...
export function evaluationConfiguration(policy, extraction = {}) {
  return {
    plugin: plugin.version,
    codeHash,
    runtime: plugin.peerDependencies['@deepseek-ai/dsh-tools'],
    policy: { ...policy },
...
export function assessEvaluation(report) {
  ...
  if (
    ...
    report.version !== 2 ||
    report.protocol !== 'session-learning-v2' ||
  ...
  if (
    ...
    report.configurationHash !== digest(report.configuration) ||
    !report.configuration.codeHash ||
    !report.configuration.runtime ||
    !report.configuration.plugin ||
    !Array.isArray(report.repetitions) ||
    report.repetitions.length < 2 ||
    new Set(report.repetitions).size !== report.repetitions.length ||
    report.trials.some(
      (t) =>
        !report.repetitions.includes(t.repetition) ||
        t.oracle !== 'withheld-exact' ||
        t.trainingFrozen !== true ||
        t.sessionProtocol !== 'harness' ||
        !Number.isSafeInteger(t.extractionTokens)
    )
  )
    fail(
      'evaluation-required',
      'Require repeated end-to-end Harness trials with pinned configuration and withheld oracles'
    )
  for (const repetition of report.repetitions)
    for (const mode of MODES)
      if (report.trials.filter((t) => t.mode === mode && t.repetition === repetition).length < 30)
        fail('evaluation-required', 'Each repetition needs 30 paired tasks in every mode')
  if (
