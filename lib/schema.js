...
const legacyEvidenceSchema = z
  ...
  .strict()
export const evidenceSchema = legacyEvidenceSchema.extend({
  kind: z.enum([
    'user-statement',
    'tool-result',
    'assistant-claim',
...
export const legacyStateSchema = z
  .object({
    ...
    evidence: z.array(legacyEvidenceSchema).max(4000),
  ...
  .strict()
export const stateSchema = legacyStateSchema.extend({
  schema: z.literal(2),
  evidence: z.array(evidenceSchema).max(4000),
  maintenance: z
    .array(z.object({ name: id, reason: str, at: time, hash: hash.optional() }))
    .max(500),
  autonomy: legacyStateSchema.shape.autonomy.extend({
    configurationHash: hash.nullable().default(null)
  }),
  factCandidates: z
    .array(
      z
    ...
    .max(1000)
    .default([]),
  scheduled: z.record(z.string(), time).default({}),
  settled: z.record(z.string(), time).default({}),
  budget: legacyStateSchema.shape.budget.extend({ unknown: time.default(0) }),
  outcomes: z
    .array(legacyStateSchema.shape.outcomes.element.extend({ seq: time.default(0) }))
    .max(4000),
  jobs: z
    .array(
      job.extend({
        reason: z.string().max(256).default('legacy-incomplete'),
        target: z.object({ name: id, hash }).optional()
      })
    )
    .max(1000)
})
export function emptyState(scope, root = '') {
  return {
    ...
    receipts: {},
    outcomes: [],
    budget: { day: '', reserved: 0, used: 0, unknown: 0 },
    autonomy: { enabled: false, evaluation: null, classes: [], configurationHash: null },
    paused: false,
    maintenance: []
  }
}
