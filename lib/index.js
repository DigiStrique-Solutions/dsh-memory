...
import { bootstrapUpgrade } from './upgrade.js'
...
export const Config = Schema.object({
  ...
  autoFacts: Schema.boolean().default(true),
...
export default class StriqueMemory extends Service {
  ...
  constructor(ctx, config) {
    ...
    this.jobs = new Set()
    this.ready = false
    this.root = join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'strique-memory-v2')
  ...
  async [Service.init]() {
    ...
    try {
      ...
      await bootstrapUpgrade(this.root, table)
      const backend = {
